...
describe('LogConsoleInterceptor', () => {
  ...
  let requestInfoCoreService: Mocked<Pick<RequestInfoCoreService, 'getUserEmail'>>;
  ...
  beforeEach(() => {
    ...
    requestInfoCoreService = { getUserEmail: vi.fn().mockReturnValue('user@x.com') };
  ...
  function buildInterceptor(option?: LogConsoleOptions, nivel: 'verbose' | 'log' | 'debug' = 'verbose') {
    ...
    logSpy = vi.spyOn((interceptor as any).logger, nivel).mockImplementation(() => undefined);
    return interceptor;
  }
  ...
  it('loga no formato do core, com method, ip, usuário, status e duração', () => {
    return new Promise<void>((done) => {
      mockLogCustom(undefined);
  ...
  it('loga no nível configurado quando nivel é informado', () => {
    return new Promise<void>((done) => {
  ...
  it('não loga o body do request quando excluirRequest é true', () => {
    return new Promise<void>((done) => {
  ...
  it('não loga o response quando excluirResponse é true', () => {
    return new Promise<void>((done) => {
  ...
  it('não loga o End de sucesso quando salvarSucesso é false', () => {
    return new Promise<void>((done) => {
  ...
  describe('habilitado / LOG_REQUEST', () => {
    it('não loga sucesso quando a env LOG_REQUEST é false', () => {
      return new Promise<void>((done) => {
    ...
    it('a opção habilitado tem prioridade sobre a env LOG_REQUEST', () => {
      return new Promise<void>((done) => {
        process.env.LOG_REQUEST = 'false';
    ...
    it('ainda loga o erro mesmo com o log de sucesso desabilitado', () => {
      return new Promise<void>((done) => {
  ...
  describe('rotasIgnoradas', () => {
    it('não loga sucesso das rotas ignoradas (por prefixo)', () => {
      return new Promise<void>((done) => {
    ...
    it('continua logando as rotas que não estão na lista', () => {
      return new Promise<void>((done) => {
    ...
    it('ainda loga o erro de uma rota ignorada', () => {
      return new Promise<void>((done) => {
        buildInterceptor(new LogConsoleOptions({ rotasIgnoradas: ['/ping'] }));
        const errorSpy = vi.spyOn((interceptor as any).logger, 'error').mockImplementation(() => undefined);
        mockLogCustom(undefined);
  ...
  describe('erro', () => {
    it('loga o erro quando ele é uma instância de Error normal (com stack)', () => {
      return new Promise<void>((done) => {
        mockLogCustom(undefined);
        const errorSpy = vi.spyOn((interceptor as any).logger, 'error').mockImplementation(() => undefined);
        const request = buildRequest();
    ...
    it('não lança um erro diferente (TypeError) e ainda loga quando o erro lançado não tem .stack', () => {
      return new Promise<void>((done) => {
        mockLogCustom(undefined);
        const errorSpy = vi.spyOn((interceptor as any).logger, 'error').mockImplementation(() => undefined);
        const request = buildRequest();
